...
const Footer = () => {
  return (
    <>
      <div className=" w-full bg-charcoal pt-8 pb-20 px-32 " id="footer">
        <LogoFooter />
        <div className="flex flex-col pl-[3.75rem] pr-4">
          ...
          {address.map((add, index) => (
            <p
              key={index}
              className="flex flex-1 text-2xl leading-5 text-white font-HN_Light border-b-2 border-gray/50 tracking-tight"
            >
              {add}
            </p>
          ))}
